...
The server proxies `/explorer-api/*` to `https://testnet-explorer.usernodelabs.org/api/*` so clients avoid CORS issues. Note that this proxy works even in `--local-dev` — see `AGENTS.md` Section 17 for how to avoid bypassing mock mode.
